...
import { POPULAR_PRODUCTS } from "../../data/homeData"
  ...
  return (
    <section className="bg-white rounded mb-3 p-3">
      <div className="flex items-center justify-between mb-2 pb-2 border-b border-gray-100">
        <h2 className="text-[15px] font-bold text-gray-900">Popular Products</h2>
        <Link to="/category/all" className="text-[12px] text-[#1a7fe8] hover:underline font-medium flex items-center gap-0.5">
            ...
            <div key={p._id} className="flex flex-col">
              <Link to={`/product/${p.slug}`} className="group block">
                <div className="bg-white border border-gray-100 rounded flex items-center justify-center overflow-hidden"
                  style={{height:"140px", padding:"6px"}}>
                  <img src={p.image} alt={p.name}
                    className="h-[126px] w-full object-contain group-hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="mt-1.5">
                  <p className="text-[11px] text-gray-800 font-medium leading-snug line-clamp-2" style={{minHeight:"26px"}}>{p.name}</p>
                  <p className="text-[12px] font-bold text-[#2e7d32] mt-0.5">{p.priceLabel}</p>
              ...
              <div className="flex gap-1.5 mt-1.5">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1 text-[11px] py-[5px] rounded border transition-colors"
                ...
                <button className="flex-1 flex items-center justify-center gap-1 text-[11px] py-[5px] rounded border border-gray-300 text-gray-600 hover:bg-gray-50 transition-colors">
